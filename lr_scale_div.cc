...
void QwtScaleDiv::invert()
{
    ...
    for ( int i = 0; i < NTickTypes; i++ )
    {
		std::vector<double>& ticks = d_ticks[i];

        const int size = ticks.size();
...
*/
void QwtScaleDiv::setTicks( int type, std::vector<double> ticks )
{
...
/*!
   ...
   \param type MinorTick, MediumTick or MajorTick
*/
std::vector<double> QwtScaleDiv::ticks( int type ) const
{
    if ( type >= 0 || type < NTickTypes )
        return d_ticks[type];

	std::vector<double> noTicks;
    return noTicks;
}
