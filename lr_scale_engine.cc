    ...
    double intervalSize )
{
    ...
    return qwtCeilF( value ) * intervalSize; //defined in lr_math.h
}
...
double QwtScaleArithmetic::floorEps( double value, double intervalSize )
{
    ...
    value = ( value + eps ) / intervalSize;
    return qwtFloorF( value ) * intervalSize; //defined in lr_math.h
}
...
double QwtScaleArithmetic::ceil125( double x )
{
    ...
    double fr = qPow( 10.0, lx - p10 ); //defined in lr_math.h
    if ( fr <= 1.0 )
        fr = 1.0;
    else if ( fr <= 2.0 )
        fr = 2.0;
...
*/
std::vector<double> QwtScaleEngine::strip( std::vector<double> ticks,
    const QwtInterval &interval ) const
{
	if ( !interval.isValid() || ticks.size() == 0 ) {
        std::vector<double> empty;
		empty.clear();
		return empty;
	}

	std::vector<double>::iterator it_beg = ticks.begin();
    std::vector<double>::iterator it_end = ticks.end();
	if ( contains( interval, *it_beg )
        && contains( interval, *it_end ) )
    {
        return ticks;
    }
    ...
    {
        if ( contains( interval, *it ) )
            strippedTicks.push_back(*it);
    }
    ...
    int maxMajSteps, int maxMinSteps, double stepSize ) const
{
    ...
    if ( stepSize != 0.0 )
    {
		...
		ticks.resize(QwtScaleDiv::NTickTypes);
        buildTicks( interval, stepSize, maxMinSteps, ticks );

        scaleDiv = QwtScaleDiv( (const QwtInterval&)interval, ticks );
    }
...
void QwtLinearScaleEngine::buildTicks(
	...
	std::vector< std::vector<double> > ticks ) const
{
	ticks.resize(QwtScaleDiv::NTickTypes);
    ...
    for ( int i = 0; i < QwtScaleDiv::NTickTypes; i++ )
    {
        ...
        for ( int j = 0; j < (int)ticks[i].size(); j++ )
        {
...
*/
std::vector<double> QwtLinearScaleEngine::buildMajorTicks(
    const QwtInterval &interval, double stepSize ) const
{
    ...
    ticks.push_back(interval.minValue());
    for ( int i = 1; i < numTicks - 1; i++ )
        ticks.push_back(interval.minValue() + i * stepSize);
    ticks.push_back(interval.maxValue());
...
void QwtLinearScaleEngine::buildMinorTicks(
	const std::vector<double> majorTicks,
    int maxMinSteps, double stepSize,
	std::vector<double> minorTicks,
	std::vector<double> mediumTicks ) const
{
    ...
    for ( int i = 0; i < (int)majorTicks.size(); i++ )
    {
        ...
        for ( int k = 0; k < numTicks; k++ )
        {
            ...
            if ( k == medIndex )
                mediumTicks.push_back(alignedValue);
            else
...
/*!
   ...
   \return Calculated ticks
*/
std::vector<double> QwtLog10ScaleEngine::buildMajorTicks(
    const QwtInterval &interval, double stepSize ) const
{
    ...
    for ( int i = 1; i < numTicks - 1; i++ )
        ticks.push_back(qExp( lxmin + double( i ) * lstep ));
...
/*!
   ...
   \param stepSize Step size
*/
std::vector<double> QwtLog10ScaleEngine::buildMinorTicks(
    ...
    int maxMinSteps, double stepSize ) const
{
    ...
    else  // major step > one decade
    {
        ...
        const double minFactor = qMax( qPow( 10.0, minStep ), double( 10.0 ) );
